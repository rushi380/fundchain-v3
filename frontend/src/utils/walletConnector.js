/**
 * walletConnector.js — Freighter v2 fixed
 */
...
export class WalletError extends Error {
  constructor(message, code, walletId) {
    super(message); this.name = 'WalletError'; this.code = code; this.walletId = walletId;
  }
}
...
export const freighterConnector = {
  ...
  async connect() {
    let api;
    try { api = await import('@stellar/freighter-api'); } catch { api = window.freighter; }
    if (!api) throw new WalletError('Could not load Freighter API.', 'LOAD_ERROR', this.id);

    let present = false;
    try { const r = await api.isConnected(); present = r?.isConnected ?? r ?? false; } catch {}
    if (!present) throw new WalletError(
    ...
    if (typeof api.requestAccess === 'function') {
      try { const r = await api.requestAccess(); address = r?.address ?? null; }
      catch (err) {
    ...
    if (!address) {
      try {
        if (typeof api.getAddress === 'function') { const r = await api.getAddress(); address = r?.address ?? r; }
        else if (typeof api.getPublicKey === 'function') { const r = await api.getPublicKey(); address = r?.publicKey ?? r; }
      } catch (err) { throw new WalletError(`Address error: ${err.message}`, 'CONNECT_ERROR', this.id); }
    }
    if (!address || !address.startsWith('G')) {
      throw new WalletError('Invalid address. Set up your Freighter wallet first.', 'NO_KEY', this.id);
    }

    try {
      let net = null;
      if (typeof api.getNetworkDetails === 'function') { const d = await api.getNetworkDetails(); net = d?.network; }
      else if (typeof api.getNetwork === 'function') { const r = await api.getNetwork(); net = r?.network ?? r; }
      if (net) {
        const isTestnet = net === 'TESTNET' || net.includes('Test SDF') || net.toLowerCase().includes('testnet');
    ...
    this.address = address; this._api = api;
    try {
      const resp = await fetch(`https://horizon-testnet.stellar.org/accounts/${address}`);
      if (resp.ok) {
        const data = await resp.json();
        const nat  = data.balances?.find(b => b.asset_type === 'native');
        this.balance = nat ? parseFloat(nat.balance).toFixed(2) : '0.00';
  ...
  async signTransaction(xdr) {
    ...
    try {
      const r = await this._api.signTransaction(xdr, {
        networkPassphrase: NETWORKS.STELLAR_TESTNET.passphrase, network: 'TESTNET',
      });
      return r?.signedTxXdr ?? r?.signedXDR ?? r;
    } catch (err) {
